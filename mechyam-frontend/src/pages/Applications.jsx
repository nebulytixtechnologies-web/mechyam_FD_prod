...
const Applications = () => {
  ...
  // pagination state (for applicants inside a job)
  const [currentPage, setCurrentPage] = useState(1);
  const pageSize = 5;

  // search inside selected job
...
if (selectedJob) {
  ...
  totalPages = Math.max(1, Math.ceil(filteredApplicants.length / pageSize));

  const startIndex = (currentPage - 1) * pageSize;
  paginatedApplicants = filteredApplicants.slice(
    startIndex,
    startIndex + pageSize
  );
}
